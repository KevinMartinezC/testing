<?php
    ...
    switch($accion){
        case "btnAgregar":
            ...
            $sentencia = $pdo->prepare("INSERT INTO emprendimientos(Nombre,Descripcion,Lugar,Fecha,Foto) VALUES (:Nombre, :Descripcion, :Lugar, :Fecha, :Foto)");
            ...
            $sentencia->execute();
            header('Location: desarrolloEconomico.php');
        break;

        case "btnModificar":
            $sentencia = $pdo->prepare("UPDATE  emprendimientos SET Nombre=:Nombre, Descripcion=:Descripcion , Lugar=:Lugar ,Fecha=:Fecha WHERE Id=:id");
            $sentencia->bindParam(':Nombre', $txtNombre);
            $sentencia->bindParam(':Descripcion', $txtDescripcion);
            $sentencia->bindParam(':Lugar', $txtLugar);
            $sentencia->bindParam(':Fecha', $txtFecha);
            ...
            if($tmpFoto!=""){
                ...
                $sentencia = $pdo->prepare("SELECT Foto FROM  emprendimientos WHERE Id=:id");
                $sentencia->bindParam(':id', $txtId);
                $sentencia->execute();
                ...
                $sentencia = $pdo->prepare("UPDATE  emprendimientos SET Foto=:Foto  WHERE Id=:id");
                $sentencia->bindParam(':Foto', $nombreArchivo);
                $sentencia->bindParam(':id', $txtId);
                $sentencia->execute();
                header('Location: desarrolloEconomico.php');
        ...
        case "btnEliminar":
            ...
            $sentencia = $pdo->prepare("DELETE FROM  emprendimientos WHERE Id=:id");
            $sentencia->bindParam(':id', $txtId);
            $sentencia->execute();
            header('Location: desarrolloEconomico.php');
        break;
        ...
        case "Seleccionar":
            ...
            $sentencia = $pdo->prepare("SELECT * FROM  emprendimientos WHERE Id=:id");
            $sentencia->bindParam(':id', $txtId);
            $sentencia->execute();
            $empleado = $sentencia->fetch(PDO::FETCH_LAZY);
    ...
    $sentencia = $pdo->prepare("SELECT * FROM  emprendimientos");
    $sentencia->execute();
